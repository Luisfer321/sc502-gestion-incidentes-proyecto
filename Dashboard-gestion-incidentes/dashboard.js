...
const incidents = [
  ...
  function loadTableData() {
    ...
    filteredIncidents.forEach((incident, index) => {
      ...
      const row = `<tr>
        ...
        <td><a href="detalle.html" class="btn btn-sm btn-primary">Ver Detalle</a></td>
      </tr>`;
      tbody.innerHTML += row;
    });
  }
  ...
  function addIncident() {
    Swal.fire({
      ...
      html: `
        <input id="incName" class="swal2-input" placeholder="Nombre del incidente">
        <select id="incSeverity" class="swal2-select">
          <option value="Alta">Alta</option>
          <option value="Media">Media</option>
          <option value="Baja">Baja</option>
        </select>
        <select id="incStatus" class="swal2-select">
  ...
  window.onload = () => {
    ...
    // Organizar los botones en una fila horizontal y con espacio
